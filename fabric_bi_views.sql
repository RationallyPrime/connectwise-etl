...
FROM gold_agreement_period_facts
...
FROM gold_agreement_summary_facts
...
FROM gold_agreement_period_facts ap
LEFT JOIN gold_agreement_summary_facts ash ON ap.id = ash.id
...
SELECT 
    ...
    COUNT(DISTINCT companyId) as unique_customers
FROM gold_invoice_header_facts
...
WITH customer_metrics AS (
    SELECT 
        ...
        AVG(actual_avg_monthly_revenue) as avg_monthly_revenue,
        SUM(CASE WHEN cancelledFlag = false THEN 1 ELSE 0 END) as active_agreements,
