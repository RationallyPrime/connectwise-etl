...
from unified_etl_core.silver.standardize import standardize_dataframe_from_config
from unified_etl_core.pipeline.bronze_silver import bronze_to_silver_transformation
...
def load_silver_config():
    ...
    with open("/lakehouse/default/Files/connectwise_silver_config.yaml", "r") as f:
...
def apply_silver_transformations(spark, entity_name, bronze_df, config):
    ...
    silver_df = typed_df \
        .withColumn("SilverCreatedAt", current_timestamp()) \
        .withColumn("SilverModifiedAt", current_timestamp())
...
def process_entity_to_silver(spark, entity_name, extraction_results, config):
    ...
    silver_table_name = config['entities'][entity_name].get('silver_table', f'silver_{entity_name.lower()}')
...
def process_all_to_silver(spark, extraction_results):
    ...
    for entity_name in extraction_results.keys():
        ...
        try:
            ...
            print(f"❌ Failed to process {entity_name}: {str(e)}")
            silver_results[entity_name] = {
                "success": False,
                "error": str(e)
            }
