...
class TestLocalPipeline:
    ...
    def test_bronze_to_silver_transformation(self, spark, test_data):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            silver_df = silver_df \
                ...
                .withColumn("actualHours", F.col("actualHours").cast(DoubleType()))
    ...
    def test_conformed_dimension_creation(self, spark, test_data):
        ...
        employee_dim = employee_dim.withColumn("employee_key", F.row_number().over(Window.orderBy("memberId")))
        employee_dim = employee_dim.withColumn("employee_name", F.concat(F.lit("Employee "), F.col("memberId")))
        
        # Add BC mapping (simulated)
        employee_dim = employee_dim.withColumn("bc_resource_no", F.concat(F.lit("RES"), F.col("memberId")))
    ...
    def test_integrated_view_query(self, spark, test_data):
        ...
        unified = unified.select(
            ...
            F.coalesce(F.col("revenue"), F.lit(0)) + F.coalesce(F.col("amount"), F.lit(0)).alias("total_amount")
        )
...
class TestPerformance:
    ...
    def test_large_dataset_processing(self, spark):
        ...
        result = df.groupBy("memberId") \
            .agg(F.sum("actualHours").alias("total_hours"),
                 F.avg("hourlyRate").alias("avg_rate")) \
            .collect()
