...
def mock_spark_session():
    ...
    with patch('fabric_api.pipeline.get_spark_session') as mock_get_spark:
        ...
        mock_df.columns = ['id', 'name', 'company_id', 'company_name', 'etl_timestamp', 'etl_entity']
...
def mock_pyspark_functions():
    ...
    with patch('fabric_api.pipeline.col') as mock_col, \
         patch('fabric_api.pipeline.lit') as mock_lit:
...
def test_bronze_extraction(cw_client, mock_spark_session, mock_pyspark_functions):
    ...
    with patch('fabric_api.pipeline.extract_entity', return_value=mock_data), \
...
def test_column_pruning_config():
    ...
    required_entities = ["Agreement", "TimeEntry", "ExpenseEntry", "ProductItem", "PostedInvoice", "UnpostedInvoice"]
...
def test_bronze_to_silver_transformation(mock_spark_session, mock_pyspark_functions):
    ...
    with patch('fabric_api.pipeline.flatten_all_nested_structures', return_value=mock_spark_session.table.return_value), \
...
def test_full_pipeline(cw_client, mock_spark_session, mock_pyspark_functions):
    ...
    with patch('fabric_api.pipeline.extract_entity', return_value=mock_data), \
         ...
         patch('fabric_api.pipeline.flatten_all_nested_structures', return_value=mock_spark_session.table.return_value):
