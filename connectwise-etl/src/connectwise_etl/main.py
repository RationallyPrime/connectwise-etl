...
def create_gold_tables_yaml(spark: SparkSession, mode: str = "full") -> None:
    """Create fact tables and dimensions using YAML schemas.

    Args:
        spark: SparkSession
        mode: "full" or "incremental" - in incremental mode, dimensions are updated not recreated
    """
    from . import transforms
    from .yaml_dimensions import create_all_dimensions_yaml

    # Create dimensions first using YAML
    logger.info(f"Creating ConnectWise dimensions from YAML (mode={mode})...")
    create_all_dimensions_yaml(spark, mode=mode)
    ...
    for fact_info in facts_to_create:
        ...
        try:
            ...
            # In incremental mode, merge facts instead of overwriting
            if mode == "incremental" and spark.catalog.tableExists(fact_info["gold_table"]):
                # Use MERGE for fact tables to update existing and insert new
                temp_view = f"temp_{fact_info['name']}_fact"
                gold_df.createOrReplaceTempView(temp_view)

                # Most facts use 'id' as the business key
                merge_keys = ["id"] if "id" in gold_df.columns else gold_df.columns[:1]
                merge_condition = " AND ".join(
                    [f"target.{key} = source.{key}" for key in merge_keys]
                )

                merge_sql = f"""
                MERGE INTO {fact_info["gold_table"]} AS target
                ...
                final_count = spark.sql(
                    f"SELECT COUNT(*) FROM {fact_info['gold_table']}"
                ).collect()[0][0]
                logger.info(
                    f"✅ Updated fact table {fact_info['gold_table']} - total records: {final_count}"
                )
            else:
                # Full refresh or table doesn't exist
...
def run_etl_pipeline(
    ...
    if "gold" in layers:
        ...
        create_gold_tables_yaml(spark, mode=mode)
