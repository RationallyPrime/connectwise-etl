...
def create_dimension_from_yaml(
    ...
    # Read source data
    if not dim_schema.source_table:
        raise ETLConfigError(
            f"Source table not defined for dimension '{dimension_name}'",
            code=ErrorCode.CONFIG_MISSING,
        )
    ...
    dim_df = (
        source_df.where(F.col(natural_key).isNotNull())
        .groupBy(natural_key)
    ...
    dim_df = dim_df.withColumn(dim_schema.primary_key, F.row_number().over(window))
    ...
    for col_schema in dim_schema.columns:
        ...
        if (
            col_name != dim_schema.primary_key
            and col_name != natural_key
            and col_name in available_columns
        ):
            # Add the column (take first non-null value)
            enriched = (
                source_df.where(F.col(col_name).isNotNull())
                .groupBy(natural_key)
    ...
    dim_df = (
        dim_df.withColumn("IsActive", F.lit(True))
...
def create_all_dimensions_yaml(
    ...
    Args:
        spark: SparkSession
        schema_dir: Directory with YAML schemas
        mode: "full" or "incremental" - in incremental, only update changed records
    """
    if not schema_dir:
        schema_dir = Path(__file__).parent / "schemas"
    ...
    for dim_name, dim_schema in dimension_schemas.items():
        logger.info(f"Processing dimension: {dim_name} (mode={mode})")
        ...
        if mode == "incremental" and spark.catalog.tableExists(table_name):
            # In incremental mode, merge changes instead of overwriting
            # For dimensions, we typically do SCD Type 1 (overwrite matching records)
            existing_df = spark.table(table_name)

            # Get the business key(s) from the dimension
            # Most dimensions use 'id' as the key, but some might differ
            merge_keys = ["id"] if "id" in dim_df.columns else dim_df.columns[:1]

            # Create temp view for merge
            temp_view = f"temp_{dim_name}_merge"
            dim_df.createOrReplaceTempView(temp_view)

            # Build merge condition
            merge_condition = " AND ".join([f"target.{key} = source.{key}" for key in merge_keys])

            # Execute MERGE (update existing, insert new)
            merge_sql = f"""
            ...
            logger.info(f"✅ Updated {table_name} - total records: {final_count}")
        else:
...
def get_dimension_mappings_from_yaml(
    ...
    for _, dim_schema in dimension_schemas.items():
        natural_key = dim_schema.natural_key
