...
def load_etl_config(
    ...
    try:
        ...
        error_msg = f"Failed to load ETL configuration from {path_to_use}: {str(e)}"
        logging.error(error_msg)
...
def get_table_config(table_base_name: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        logging.error(f"Unexpected error getting config for {table_base_name}: {str(e)}")
        return None
...
def get_all_table_configs() -> dict[str, Any]:
    ...
    try:
        ...
        logging.error(f"Unexpected error getting all configs: {str(e)}")
        return {}
