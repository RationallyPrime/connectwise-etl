...
from bc_etl.utils.exceptions import AnalysisException
...
def write_with_schema_conflict_handling(
    ...
    try:
        ...
        logging.warning(f"Error checking table existence for {table_path}: {str(e)}")
    ...
    try:
        ...
        if post_write_sql:
            try:
                ...
                logging.error(f"Failed to execute post-write SQL on {table_path}: {str(sql_e)}")
            ...
            try:
                ...
                if post_write_sql:
                    try:
                        ...
                        logging.error(
                            f"Failed to execute post-write SQL on recreated {table_path}: {str(sql_e)}"
                        )
                ...
                logging.error(
                    f"Failed to recreate/rewrite table {table_path} after conflict: {str(recreate_e)}"
                )
