...
from bc_etl.gold.facts import (
    ...
    create_item_fact,
    create_jobs_fact,
    create_purchase_fact,  # Consolidated import
    create_sales_fact,
)
from bc_etl.gold.hierarchy import build_account_hierarchy
from bc_etl.gold.keys import generate_surrogate_key
from bc_etl.pipeline.writer import write_with_schema_conflict_handling
from bc_etl.silver.standardize import get_silver_table_name
from bc_etl.utils import (
    construct_table_path,
    logging,
    watermark_manager,
)
from bc_etl.utils.config_loader import get_all_table_configs, get_table_config
from bc_etl.utils.exceptions import ConfigurationError
...
def silver_to_gold_transformation(
    ...
    if not is_full_refresh:
        try:
            ...
            logging.warning(f"Failed to get watermark for {gold_full_path}: {str(e)}")
    ...
    with logging.span(
        ...
        if table_base_name == "DateDimension":
            try:
                ...
                from bc_etl.gold.dimensions import generate_date_dimension  # Deferred import
                ...
                logging.error(f"Failed to generate date dimension: {str(e)}")
                return False
        ...
        try:
            ...
            logging.error(f"Failed to read silver table {silver_full_path}: {str(e)}")
            return False
        ...
        try:
            ...
            # --- Fact Handling ---
                ...
                # Check if the table is appropriate for the target fact
                if target_fact_name == "fact_Finance":
                    # Finance fact uses ledger entries
                ...
                elif target_fact_name == "fact_Inventory":
            ...
            # --- Bridge/Dimension Framework Handling ---
                ...
                from bc_etl.gold.dimensions import create_dimension_bridge  # Deferred import
                ...
                from bc_etl.gold.dimensions import create_item_attribute_bridge  # Deferred import

                gold_df = create_item_attribute_bridge(spark, silver_db, gold_db)
                write_mode = "overwrite"  # Bridges are overwritten

            elif table_base_name == "ItemAttributeValue":
                from bc_etl.gold.dimensions import (
            ...
            logging.error(
                f"Failed gold transformation for {table_base_name} -> {gold_target_name}: {str(e)}",
                exc_info=True,
            )
            return False
