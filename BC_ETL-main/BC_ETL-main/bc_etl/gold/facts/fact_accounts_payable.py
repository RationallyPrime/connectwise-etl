"""
Accounts Payable fact table creation module.
Handles the transformation of VendorLedgerEntry data into the fact_AccountsPayable gold table.
"""
...
# Define Target Schema for accounts payable fact table
FACT_ACCOUNTS_PAYABLE_SCHEMA = StructType(
    [
        StructField("$Company", StringType(), True),
        StructField("EntryNo", IntegerType(), True),
        StructField("VendorNo", StringType(), True),
        StructField("VendorKey", IntegerType(), True),  # From dim join
        ...
        StructField("VendorPostingGroup", StringType(), True),  # Posting group
...
@fact_table_handler(error_msg="Failed to create accounts payable fact table")
def create_accounts_payable_fact(
    ...
    """
    Create accounts payable fact table from Vendor Ledger Entry table.
    ...
    Returns:
        DataFrame containing the accounts payable fact table

    Raises:
        FactTableError: If required columns are missing or processing fails
    """
    with logging.span("create_accounts_payable_fact"):
        logging.info("Starting create_accounts_payable_fact")
        ...
        table_base_name = source_table or "VendorLedgerEntry"
        ...
        if incremental:
            last_processed_time = manage_watermark(
                ...
                default_lookback_days=90,  # Default to 90 days for AP entries
            )

        # Read Vendor Ledger Entry table using table_utils
        vendor_df, vendor_table = read_table_from_config(
            spark=spark,
        ...
        # Ensure vendor_df is not None before proceeding
        ...
        try:
            ...
            fact_df = fact_df.join(
                ...
                & (fact_df["VendorNo"] == bridge_df["VendorNo"])
                & (bridge_df["SourceTable"] == "Vendor"),
                "left",
            )
        ...
        result_df = enforce_schema(
            df=fact_df,
            target_schema=FACT_ACCOUNTS_PAYABLE_SCHEMA,
            cast_columns=True,
            add_missing=True,
        )
        ...
        logging.info(f"Created accounts payable fact with {result_df.count()} rows")
        return result_df
