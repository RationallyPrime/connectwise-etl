    ...
    try:
        with logging.span("create_bc_dimension_bridge"):
            ...
            try:
                ...
                raise DimensionResolutionError(f"Failed to load BC dimension tables: {e}") from e
        ...
        raise DimensionResolutionError(f"BC dimension bridge creation failed: {e}") from e
...
def create_bc_item_attribute_dimension(
    ...
    try:
        with logging.span("create_bc_item_attribute_dimension"):
            ...
            try:
                ...
                raise DimensionResolutionError(
                    f"Failed to read BC item attribute tables: {e}"
                ) from e
        ...
        raise DimensionResolutionError(f"BC item attribute dimension creation failed: {e}") from e
...
def join_bc_dimension(
    ...
    """
    if not spark:
        raise DimensionJoinError("SparkSession is required")
    if not fact_df:
        raise DimensionJoinError("fact_df is required")
    if not dimension_name:
        raise DimensionJoinError("dimension_name is required")
    if not fact_join_keys:
        raise DimensionJoinError("fact_join_keys is required")
    if not gold_path:
        raise DimensionJoinError("gold_path is required")
    ...
    try:
        with logging.span("join_bc_dimension", dimension_name=dimension_name):
            ...
            if not dim_target_table:
                raise DimensionJoinError(f"gold_name required for BC dimension {dimension_name}")
        ...
        if not isinstance(e, DimensionJoinError):
            raise DimensionJoinError(f"BC dimension join failed for {dimension_name}: {e}") from e
        raise
    ...
    try:
        with logging.span("build_bc_account_hierarchy"):
            ...
            if indentation_col is None:
                ...
                if not indent_cols:
                    raise HierarchyBuildError("No BC indentation column found")
            ...
            if no_col not in df.columns:
                ...
                if potential_cols:
                    ...
                    raise HierarchyBuildError(f"BC account number column '{no_col}' not found")
        ...
        raise HierarchyBuildError(f"BC account hierarchy build failed: {e}") from e
