...
def process_integration(
    ...
    if "bronze" in layers:
        ...
        if extractor:
            try:
                # Bronze: Extract raw data and store as-is
        ...
        if models:
            try:
                ...
                for entity_name, model_class in models.items():
                    ...
                    try:
                        # Handle fully qualified table names (e.g., Lakehouse.bronze.bronze_cw_agreement)
                        if "." in bronze_table:
                            bronze_df = spark.table(bronze_table)
                        else:
                            # Try with Lakehouse.bronze prefix if not fully qualified
                            try:
                                bronze_df = spark.table(f"Lakehouse.bronze.{bronze_table}")
                            except:
                        ...
                        if "." in silver_table:
                            ...
                            silver_df.write.mode("overwrite").saveAsTable(
                                f"Lakehouse.silver.{silver_table}"
                            )
