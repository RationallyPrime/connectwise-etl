...
from unified_etl.bronze.reader import read_bc_table
...
from unified_etl.silver.standardize import get_silver_table_name, standardize_dataframe_from_config
...
def get_incremental_filter(
    ...
    if table_base_name:
        try:
            ...
            logging.warning(f"Error getting incremental column from config: {e!s}")
...
def create_empty_table_if_not_exists(
    ...
    try:
        ...
        try:
            ...
            logging.error(f"Failed to create empty table {full_table_path}: {e!s}")
            raise
...
def bronze_to_silver_transformation(
    ...
    with logging.span(
        ...
        try:
            ...
            logging.error(f"Failed to read bronze data: {e!s}")
            return False, None
        ...
        try:
            ...
            logging.error(f"Failed to standardize column names: {e!s}")
        ...
        try:
            ...
            logging.error(f"Failed to apply data types: {e!s}")
        ...
        try:
            ...
            logging.error(f"Failed to apply SCD handling: {e!s}")
            processed_df = typed_df
        ...
        try:
            ...
            logging.error(
                f"Failed during write/merge operation for {silver_full_path}: {e!s}",
                exc_info=True,
            )  # Add traceback
            return False, None
