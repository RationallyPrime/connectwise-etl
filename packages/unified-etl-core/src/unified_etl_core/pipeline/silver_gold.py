    ...
    try:
        with logging.span("process_silver_to_gold_table", table_name=table_base_name):
            ...
            try:
                ...
                logging.info(
                    f"Found silver table {silver_db}.{silver_table_name} with {silver_df.count()} rows"
                )
            except Exception as e:
                logging.warning(f"Silver table {silver_db}.{silver_table_name} not found: {e!s}")
                return False
            ...
            gold_df = create_fact_table(
                spark=spark, silver_df=silver_df, entity_name=table_base_name, gold_path=gold_db
            )
            ...
            if write_success:
                logging.info(
                    f"Successfully wrote {table_base_name} to gold layer as {gold_table_name}"
                )
        ...
        logging.error(f"Error processing {table_base_name} from silver to gold: {e!s}")
        return False
    ...
    if any(table.startswith("GL") or table == "GLAccount" for table in tables):
        try:
            ...
            try:
                ...
                build_account_hierarchy(gl_accounts_df)
            ...
            logging.error(f"✗ Failed to build account hierarchy: {e!s}")

    # Summary
    successful_count = sum(results.values())
    total_count = len(results)
    logging.info(
        f"Pipeline completed: {successful_count}/{total_count} tables processed successfully"
    )
...
def process_dimension_table(
    ...
    try:
        with logging.span("process_dimension_table", table_name=table_base_name):
            ...
            if key_column not in dim_df.columns:
                dim_df = generate_surrogate_key(
                    ...
                    key_name=key_column,
                    business_keys=[
                        col for col in dim_df.columns if col.lower() in ["id", "no", "code", "name"]
                    ][:2],
                )
        ...
        logging.error(f"Error processing dimension {table_base_name}: {e!s}")
        return False
