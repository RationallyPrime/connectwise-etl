...
def create_agreement_period_fact(
    ...
    period_facts = (
        period_facts.withColumn(
            "AgreementPeriodSK", F.sha2(F.concat_ws("_", "id", "period_start"), 256)
        )
        .withColumn("AgreementSK", F.sha2(F.col("id").cast("string"), 256))
        .withColumn("DateSK", F.date_format("period_start", "yyyyMMdd").cast("int"))
    )
...
def create_agreement_summary_fact(
    spark: SparkSession, agreement_df: DataFrame, period_fact_df: DataFrame | None = None
) -> DataFrame:
    """
    ...
    summary_df = (
        ...
        .withColumn("lifetime_months", F.round(F.col("lifetime_days") / 30.44, 1))
        .withColumn(
            "is_active",
        ...
        .withColumn("estimated_lifetime_value", F.col("billAmount") * F.col("lifetime_months"))
    )
    ...
    summary_df = summary_df.withColumn("AgreementSK", F.sha2(F.col("id").cast("string"), 256))
