...
from .core.pipeline_utils import log_dataframe_info, log_error, log_info, log_warning
...
class BCMedallionPipelineEnhanced:
    ...
    def bronze_to_silver(
        ...
        silver_df._model_class = model_class  # type: ignore[attr-defined]
        return silver_df
    ...
    def generate_surrogate_key(
        ...
        log_info(
            f"Generated surrogate key {key_name} for {table_name} using model {model_class.__name__ if model_class else 'None'}"
        )
        return result_df
            ...
            if table_name in silver_tables_created:
                try:
                    ...
                    if gold_dim_df:
                        ...
                        # Record successful dimension processing with model class name
                        model_cls = self.model_mapping.get(table_name)
                        results["phases"]["dimensions"][table_name] = {
                            "status": "success",
                            "table": gold_table,
                            "model_class": model_cls.__name__ if model_cls else None,
                        }
                except Exception as e:
            ...
            if table_name in silver_tables_created:
                try:
                    ...
                    if gold_fact_df:
                        ...
                        # Record successful fact processing with model class name
                        model_cls = self.model_mapping.get(table_name)
