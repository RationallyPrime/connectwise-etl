    ...
    **params: Any,
    ...
    try:
        ...
        for i in range(0, len(processed_invoice_ids), batch_size):
            ...
            logger.info(f"Fetching time entries batch {i//batch_size + 1}/{(len(processed_invoice_ids) + batch_size - 1)//batch_size}")
            ...
            for entry in batch_entries:
                try:
                    ...
                    if time_entry:
                        ...
                        if not hasattr(time_entry, "agreement_type") or time_entry.agreement_type is None or "Tímapottur" not in time_entry.agreement_type:
        ...
        logger.info(f"Successfully processed {len(all_time_entries)} time entries across all invoices")
    ...
    try:
        ...
        for i in range(0, len(processed_invoice_ids), batch_size):
            ...
            logger.info(f"Fetching expenses batch {i//batch_size + 1}/{(len(processed_invoice_ids) + batch_size - 1)//batch_size}")
            batch_entries = client.paginate(
            ...
            for entry in batch_entries:
                try:
                    ...
                    if "member" in entry and isinstance(entry["member"], dict):
                        expense_data["employee"] = entry["member"].get("identifier", "")[:10]  # Limit to 10 chars
                    else:
                        ...
                        try:
                            ...
                            if isinstance(date_str, str):
                                if "T" in date_str:
                                    ...
                                    expense_data["work_date"] = date_part  # Just the date "2018-10-03"
                                else:
                    ...
                    if isinstance(agreement_data, dict) and "id" in agreement_data:
                        ...
                        expense_data["agreement_number"] = agreement_data.get("name", "")[:10]  # Limit to 10 chars
                        
                        # Try to get parent agreement and type
                        try:
                            parent_id, agr_type = get_parent_agreement_data(client, agreement_data["id"])
                            expense_data["parent_agreement_id"] = parent_id
                            expense_data["agreement_type"] = agr_type[:50] if agr_type else None  # Limit to 50 chars
                        except Exception as e:
        ...
        logger.info(f"Successfully processed {len(all_expenses)} expense entries across all invoices")
    ...
    try:
        ...
        for i in range(0, len(processed_invoice_ids), batch_size):
            ...
            logger.info(f"Fetching products batch {i//batch_size + 1}/{(len(processed_invoice_ids) + batch_size - 1)//batch_size}")
            batch_entries = client.paginate(
        ...
        logger.info(f"Successfully processed {len(all_products)} product entries across all invoices")
    ...
    for entry in time_entries:
        ...
        try:
            ...
            line_data = {
                ...
                "memo": (getattr(entry, "notes", "") or getattr(entry, "internal_notes", ""))[:250],  # Limit to 250 chars
                "price": float(getattr(entry, "hourly_rate", 0.0)) if hasattr(entry, "hourly_rate") else None,
                "cost": float(getattr(entry, "cost_rate", 0.0)) if hasattr(entry, "cost_rate") else None,
    ...
    for expense in expenses:
        ...
        try:
            ...
            line_data = {
                "invoice_number": invoice_number,
                "line_no": len(invoice_lines) + 1,  # Use sequential line numbers
                "description": f"Expense: {getattr(expense, 'type', 'Expense')}"[:50],  # Limit to 50 chars
                "time_entry_id": None,  # Not a time entry
                "quantity": quantity,
                "line_amount": amount,
                "memo": (getattr(expense, "notes", "") or getattr(expense, "description", ""))[:250],  # Limit to 250 chars
                "price": price,
                "cost": getattr(expense, "cost", amount),  # Use amount as cost if not available
                "product_id": None,  # Not a product
                "item_identifier": getattr(expense, "identifier", None),  # Use expense identifier if available
    ...
    for product in products:
        ...
        try:
            ...
            line_data = {
                ...
                "item_identifier": getattr(product, "identifier", None) or getattr(product, "catalog_id", None),
