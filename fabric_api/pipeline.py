    ...
    try:
        ...
        for agreement in all_agreements:
            ...
            if agreement_id:
                ...
                agreement_obj, errors = extract_agreements.get_agreement_with_relations(client, agreement_id)
                if agreement_obj:
        ...
        for agreement_id in agreement_ids:
            try:
                agreement, errors = extract_agreements.get_agreement_with_relations(client, agreement_id)
                if agreement:
...
def join_invoice_data(
    *,
    ...
    headers_df: DataFrame | None = read_table_safely(spark=spark, full_table_path=invoice_headers_path, default_value=empty_df)
    ...
    lines_df: DataFrame | None = read_table_safely(spark=spark, full_table_path=invoice_lines_path, default_value=empty_df)
    ...
    times_df: DataFrame | None = read_table_safely(spark=spark, full_table_path=time_entries_path, default_value=empty_df)
    ...
    expenses_df: DataFrame | None = read_table_safely(spark=spark, full_table_path=expenses_path, default_value=empty_df)
    ...
    products_df: DataFrame | None = read_table_safely(spark=spark, full_table_path=products_path, default_value=empty_df)
    ...
    try:
        ...
        try:
            ...
            error_df = spark.createDataFrame([("Error executing SQL join", str(e))], ["error_type", "error_message"])
            return error_df
            
    except Exception as e:
        logger.error(f"Error preparing or validating tables for SQL join: {str(e)}")
        error_df = spark.createDataFrame([("Error preparing SQL join", str(e))], ["error_type", "error_message"])
        return error_df