...
def create_accounts_payable_fact(
    spark: SparkSession,
    primary_silver_df: DataFrame, # e.g., VendorLedgerEntry
    ...
    dimension_bridge_df: Optional[DataFrame], # If VendorLedgerEntry has DimensionSetID
    ...
    """
    Creates the Accounts Payable Fact table from VendorLedgerEntry and related dimensions.
    BCPipeline.silver_to_gold_fact is responsible for reading sources and writing output.
    """
    etl_logger.info("Starting creation of Accounts Payable Fact table (create_accounts_payable_fact function).")

    if primary_silver_df is None:
        etl_logger.warning("Primary silver DataFrame (e.g., VendorLedgerEntry) is None for AP Fact. Skipping.")
        return None
    ...
    if min_year_filter and "PostingDate" in fact_df.columns:
        ...
        etl_logger.info(f"AP Fact: Applied min_year_filter >= {min_year_filter} on PostingDate.")
        ...
        for date_col, key_col_name in date_columns_to_key.items():
            if date_col in fact_df.columns:
                ...
                fact_df = fact_df.join(
                    date_dim_df.alias(f"dd_join_{date_col}"), # Unique alias for each join instance
                    F.to_date(fact_df[date_col]) == F.to_date(F.col(f"dd_join_{date_col}.Date")),
                    "left"
                ).select(*select_exprs)
                etl_logger.info(f"AP Fact: Joined with Date Dimension for {date_col} as {key_col_name}.")
            else:
                etl_logger.warning(f"AP Fact: Date column '{date_col}' not found. Missing {key_col_name}.")
                fact_df = fact_df.withColumn(key_col_name, F.lit(None).cast("int"))
    else:
        etl_logger.warning("Date dimension not provided for AP Fact. Date keys will be missing.")
        for key_col_name in date_columns_to_key.values():
            fact_df = fact_df.withColumn(key_col_name, F.lit(None).cast("int"))

    # Join with Vendor Dimension
    if vendor_dim_df and "VendorNo" in fact_df.columns:
        vendor_dim_df_aliased = vendor_dim_df.alias("vdim")
        join_keys_vendor = [fact_df["VendorNo"] == F.col("vdim.No")]
        if "Company" in fact_df.columns and "Company" in vdim.columns: # Assuming Vendor dim can be company specific
        ...
        etl_logger.warning("Vendor dimension or 'VendorNo' not available for AP Fact. Missing VendorKey.")
    ...
    if dimension_bridge_df and "DimensionSetID" in fact_df.columns:
        ...
        etl_logger.warning("Dimension Bridge or 'DimensionSetID' not available for AP Fact. Missing DimensionBridgeKey.")
    ...
    etl_logger.info("Accounts Payable Fact table (create_accounts_payable_fact) processing complete.")
    return fact_df
