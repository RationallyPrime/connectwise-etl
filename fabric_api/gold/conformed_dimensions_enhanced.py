...
class ConformedDimensionBuilder:
    ...
    def create_customer_dimension(
        ...
        customer_dim = (
            customers_with_bc
            .withColumn("customer_key", row_number().over(window))
    ...
    def create_employee_dimension(
        ...
        employee_dim = (
            employees_with_bc
            .withColumn("employee_key", row_number().over(window))
            .withColumn("employee_code", coalesce(col("bc_resource_no"), col("psa_identifier")))
            .withColumn("is_integrated", when(col("bc_resource_no").isNotNull(), True).otherwise(False))
            .withColumn("processed_timestamp", current_timestamp())
        )
    ...
    def create_agreement_dimension(
        ...
        agreement_dim = (
            agreements_with_bc
            .withColumn("agreement_key", row_number().over(window))
    ...
    def create_work_type_dimension(
        ...
        work_type_dim = (
            work_types_with_bc
            .withColumn("work_type_key", row_number().over(window))
            .withColumn("is_integrated", when(col("bc_work_type_code").isNotNull(), True).otherwise(False))
            .withColumn("processed_timestamp", current_timestamp())
        )
