...
def create_customer_dimension(
    ...
    customer_dim = customers_with_bc.withColumn(
        "customer_key",
        row_number().over(window)
    ).withColumn(
        "customer_code",
        coalesce(col("bc_customer_no"), col("psa_identifier"))
    ).withColumn(
        "is_integrated",
        when(col("bc_customer_no").isNotNull(), True).otherwise(False)
    )
...
def create_employee_dimension(
    ...
    employee_dim = employees_with_bc.withColumn(
        ...
        "employee_code",
        coalesce(col("bc_resource_no"), col("psa_identifier"))
    )
...
def create_agreement_dimension(
    ...
    agreement_dim = agreements_with_bc.withColumn(
        ...
        "job_code",
        coalesce(col("bc_job_no"), col("agreement_number"))
    )
...
def create_work_type_dimension(
    ...
    work_type_dim = work_types_with_bc.withColumn(
        "work_type_key",
        row_number().over(window)
    )
...
def run_conformed_dimensions(
    ...
    psa_companies = spark.table(f"{silver_path}.PostedInvoice").select("companyId", "companyName").distinct()
    psa_members = spark.table(f"{silver_path}.TimeEntry").select("memberId", "memberName", "memberIdentifier").distinct()
    psa_work_types = spark.table(f"{silver_path}.TimeEntry").select("workTypeId", "workTypeName").distinct()
