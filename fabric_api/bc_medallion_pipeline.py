...
class BCMedallionPipeline:
    ...
    def create_date_dimension(
        ...
        date_df = date_df.withColumn(
            ...
            ).otherwise(F.make_date(F.col("Year") - 1, F.lit(fiscal_year_start_month), F.lit(1))),
        )
    ...
    def create_dimension_bridge(
        self, silver_path: str, gold_path: str, dimension_types: dict[str, str] | None = None
    ) -> DataFrame:
        """
        ...
        for dim_type, dim_code in dimension_types.items():
            ...
            result_df = result_df.withColumn(
                flag_col, F.when(F.col(code_col).isNotNull(), True).otherwise(False)
            )
    ...
    def silver_to_gold_dimension(self, table_name: str, silver_path: str) -> DataFrame | None:
    ...
    def silver_to_gold_fact(
        self, table_name: str, silver_path: str, gold_path: str, min_year: int | None = None
    ) -> DataFrame | None:
        ...
        if table_name == "GLEntry":
            ...
            try:
                ...
                log_error(f"Failed to join with GL Account dimension: {e!s}")
            ...
            try:
                ...
                log_error(f"Failed to join with Customer dimension: {e!s}")
            ...
            try:
                ...
                log_error(f"Failed to join with Vendor dimension: {e!s}")
    ...
    def run_pipeline(
        ...
        for table_name in tables:
            ...
            try:
                silver_df = self.bronze_to_silver(table_name, bronze_path, incremental=incremental)
                if silver_df:
                    ...
                    silver_df.write.mode(write_mode).option("mergeSchema", "true").saveAsTable(
                        silver_table_path
                    )
        ...
        if all(dim in silver_tables_created for dim in ["DimensionSetEntry", "DimensionValue"]):
            dimension_bridge = self.create_dimension_bridge(silver_path, gold_path, dimension_types)
            if dimension_bridge:
            ...
            if table_name in silver_tables_created:
                try:
                    gold_fact_df = self.silver_to_gold_fact(
                        table_name, silver_path, gold_path, min_year=min_year
                    )
                    if gold_fact_df:
