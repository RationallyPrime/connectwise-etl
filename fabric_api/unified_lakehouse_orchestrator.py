...
class UnifiedLakehouseOrchestrator:
    ...
    def run_bc_bronze_to_silver(
        self, tables: list[str] | None = None, incremental: bool = False
    ) -> dict[str, Any]:
        ...
        for table in tables:
            try:
                silver_df = self.bc_pipeline.bronze_to_silver(
                    table_name=table, bronze_path=self.schemas["bronze_bc"], incremental=incremental
                )
                ...
                if silver_df:
                    ...
                    results[table] = {"status": "success", "silver_rows": silver_df.count()}
                else:
    ...
    def run_conformed_dimensions(self, include_bc_data: bool = True) -> dict[str, int]:
    ...
    def run_bc_gold_processing(
        self, dimension_types: dict[str, str] | None = None, min_year: int | None = None
    ) -> dict[str, Any]:
        ...
        for dim_table in dimension_tables:
            try:
                gold_dim_df = self.bc_pipeline.silver_to_gold_dimension(
                    table_name=dim_table, silver_path=self.schemas["silver_bc"]
                )
                if gold_dim_df:
    ...
    def run_full_pipeline(
        ...
        incremental: bool = False,
        psa_entities: list[str] | None = None,
        bc_tables: list[str] | None = None,
