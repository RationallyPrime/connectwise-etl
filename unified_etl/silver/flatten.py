    ...
    def _flatten_struct_columns(df: DataFrame, prefix: str = "", depth: int = 0) -> DataFrame:
        ...
        for struct_col in struct_columns:
            ...
            for field in struct_fields:
                new_col_name = f"{prefix}{struct_col}_{field.name}" if prefix else f"{struct_col}_{field.name}"
                result_df = result_df.withColumn(
                    new_col_name, 
                    F.col(f"{struct_col}.{field.name}")
                )
...
def _identify_json_columns(df: DataFrame) -> List[str]:
    ...
    for col_name in df.columns:
        if df.schema[col_name].dataType == StringType():
            ...
            sample_values = [row[col_name] for row in sample_df.select(col_name).collect() 
