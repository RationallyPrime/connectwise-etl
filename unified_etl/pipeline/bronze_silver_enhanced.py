...
class SilverTransformation:
    ...
    def transform_bronze_to_silver(
        ...
        apply_scd: bool = True,
        scd_type: int = 2,
        prefixes_to_strip: Optional[List[str]] = None,
...
def run_enhanced_bronze_silver_pipeline(
    spark: SparkSession,
    lakehouse_root: str,
    entity_configs: Optional[List[dict]] = None
) -> bool:
    """
    ...
    for config in entity_configs:
        try:
            ...
            logging.error(f"Enhanced silver processing failed for {config.get('entity_name', 'unknown')}: {e}")
            continue
