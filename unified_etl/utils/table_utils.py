# unified_etl/utils/table_utils.py
...
from unified_etl.utils.naming import construct_table_path, standardize_table_reference
...
def read_table_from_config(
    ...
    try:
        ...
        error_msg = f"Failed to read table {full_table_path}: {e!s}"
...
def get_table_stats(
    ...
    try:
        ...
        try:
            ...
            logging.debug(f"Could not get detailed metadata: {e!s}")
        ...
        logging.error(f"Error getting table stats for {full_table_path}: {e!s}")
