...
from unified_etl.gold.date_dimension import add_date_dimension_joins, create_or_update_date_dimension_table
...
class GoldLayerEnhancer:
        ...
        for key_config in surrogate_keys:
            ...
            if key_type == "hash":
                ...
                result_df = result_df.withColumn(
                    key_name, 
                    F.row_number().over(F.Window.orderBy(*source_columns))
                )
            ...
            try:
                ...
                if isinstance(col_expression, str):
                    ...
                    if calc_type == "expression":
                        result_df = result_df.withColumn(
                            col_name, 
                            F.expr(col_expression.get("sql", "NULL"))
                        )
    ...
    def _add_account_hierarchy(self, df: DataFrame) -> DataFrame:
        ...
        if "accountNumber" in df.columns:
            ...
            df = df.withColumn(
                "AccountLevel1", 
                F.substring(F.col("accountNumber"), 1, 1)
            ).withColumn(
                "AccountLevel2", 
                F.substring(F.col("accountNumber"), 1, 3)
            ).withColumn(
                "AccountLevel3", 
                F.substring(F.col("accountNumber"), 1, 5)
            )
    ...
    def _add_business_metadata(self, df: DataFrame, entity_name: str) -> DataFrame:
        ...
        return df.withColumn("EntityType", F.lit(entity_name)) \
                .withColumn("GoldProcessedAt", F.current_timestamp()) \
                .withColumn("DataQualityScore", F.lit(100))  # Placeholder
    ...
    def _get_default_entity_config(self, entity_name: str) -> Dict[str, Any]:
        ...
        default_configs = {
            ...
            "Invoice": {
                "surrogate_keys": [
                    {"name": "InvoiceSK", "source_columns": ["id"], "type": "hash"}
                ],
        ...
        return default_configs.get(entity_name, {
            "surrogate_keys": [
...
def run_enhanced_gold_pipeline(
    ...
    lakehouse_root: str,
    entities: List[str],
    entity_configs: Optional[Dict[str, Dict[str, Any]]] = None
) -> bool:
    """
    ...
    for entity_name in entities:
        try:
            ...
            gold_df = enhancer.enhance_silver_to_gold(
                silver_df, 
                entity_name, 
                entity_configs.get(entity_name)
            )
    ...
    logging.info(f"Enhanced gold pipeline complete: {success_count}/{len(entities)} entities processed")
