...
def cdm_to_jsonschema(cdm_data: dict) -> dict:
    ...
    if class_name_match:
        clean_name = class_name_match.group(1)
    else:
...
def generate_bc_models(cdm_dir: str, output_file: str, package_dir: str):
    ...
    for schema_file in schema_files:
        ...
        cmd = [
            ...
            "--strict-nullable",
            "--use-union-operator",
            "--input", str(schema_file.absolute()),
            "--output", str(temp_model_file),
        ]
...
def main():
    if len(sys.argv) < 4:
        ...
        print("Examples:")
        print("  PSA: python regenerate_models.py psa PSA_OpenAPI_schema.json packages/unified-etl-connectwise/src/unified_etl_connectwise/models/models.py")
        print("  BC:  python regenerate_models.py bc cdm_manifests packages/unified-etl-businesscentral/src/unified_etl_businesscentral/models/models.py")
        sys.exit(1)
